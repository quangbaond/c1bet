...
const submitForm = async (formEl: any) => {
  ...
  formEl.validate(async (valid: boolean) => {
    if (valid) {
      await store.dispatch("userStore/login", form.value);
    }
  });
};
  ...
  <div id="login_wrap" class="h-dvh w-screen bg-no-repeat bg-cover" :style="`background-image: url(${bgLogin});`">
    <div class="flex justify-between px-6 py-6">
      <div class="text-white text-lg">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="w-6 h-6" @click="() => router.push('/')">
    ...
    <div class="flex justify-center items-center h-4/5">
      <el-card class="box-card w-9/12 bg-gray" style="background: rgba(214, 218, 228, 0.95)">
        <div class="login-img">Đăng nhập</div>
        <el-form ref="formRef" :model="form" class="demo-ruleForm">
          ...
          <el-form-item prop="password" :rules="[
            ...
            <el-input type="password" v-model="form.password" autocomplete="off" size="large" placeholder="Mật khẩu"
              :show-password="true">
              <template #prefix>
                <el-icon class="el-input__icon">
                  <lock />
          ...
          <el-form-item>
            <el-checkbox v-model="form.remmber" size="large">Lưu</el-checkbox>
            <el-link type="primary" @click="() => router.push('/forgot-password')">Quên mật khẩu?</el-link>
          </el-form-item>
          <el-form-item>
          ...
          <el-form-item>
            <el-button class="w-full ml-0" size="large" type="secondary" @click="() => router.push('/register')">
              Đăng ký ngay
            </el-button>
          </el-form-item>
        </el-form>
